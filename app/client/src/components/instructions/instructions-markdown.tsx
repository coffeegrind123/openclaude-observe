...
interface InstructionsMarkdownProps {
  ...
  /** Called for a local `.md` link; return false when it resolves to nothing. */
  onFileLink?: (href: string) => boolean | void
  className?: string
}
...
const LOCAL_MD_HREF = /^(?![a-z][a-z0-9+.-]*:)[^?#]+\.md(?:#.*)?$/i
...
const mdComponents = (
  ...
  a: ({ href, children, ...props }) => {
    ...
    if (href && onFileLink && LOCAL_MD_HREF.test(href)) {
      return (
        <button
          type="button"
          onClick={() => onFileLink(href)}
          className="inline rounded px-1 -mx-0.5 font-medium text-primary hover:bg-primary/10"
          title={`Open ${href}`}
        >
          {children}
        </button>
...
export function InstructionsMarkdown({
  ...
  onFileLink,
  className,
}: InstructionsMarkdownProps) {
  const rewritten = React.useMemo(() => rewriteWikilinks(body), [body])
  const components = React.useMemo(
    () => mdComponents(onNavigate, knownStems, onFileLink),
    [onNavigate, knownStems, onFileLink],
  )
  return (
