...
function findHighlightedEvent(
  ...
  for (let i = 0; i < events.length; i++) {
    const e = events[i]
    // Our ParsedEvent already carries the derived tool name; the matcher's
    // RawEvent maps our `subtype` onto `hookName`.
    const toolName = e.toolName ?? null
    const raw = { hookName: e.subtype, payload: e.payload }
    const out = applyFilters(raw, toolName, [compiledFilter], eventStrings[i])
    ...
    const highlights: PatternHighlight[] = rawPatterns.map((rp, patternIdx) => {
      ...
      const target = rp.target === 'hook' ? (e.subtype ?? '') : (toolName ?? '')
      if (r.test(target)) {
...
function LivePreview({
  ...
  const result = useMemo<{ count: number | null; highlighted: HighlightedEvent | null }>(() => {
    ...
    for (let i = 0; i < events.length; i++) {
      const e = events[i]
      // Our ParsedEvent already carries the derived tool name; the
      // matcher's RawEvent maps our `subtype` onto `hookName`.
      const toolName = e.toolName ?? null
      const raw = { hookName: e.subtype, payload: e.payload }
      const out = applyFilters(raw, toolName, [compiled], eventStrings[i])
