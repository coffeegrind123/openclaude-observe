...
//   - still flips identity for the streaming-rebuild tests, because each
//     setMockEvents call assigns a new array.
...
// The server's event shape: the row's header reads subtype and toolName.
function makeEvent(id: number, payload: Record<string, unknown>): ParsedEvent {
  ...
  return {
    ...
    agentId: 'agent-1',
    sessionId: 'sess-1',
    type: toolName ? 'tool' : 'system',
    subtype: 'PreToolUse',
    toolName,
    toolUseId: toolName ? `tu-${id}` : null,
    status: 'pending',
