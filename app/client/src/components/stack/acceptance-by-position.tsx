  ...
  return (
    <figure>
      <figcaption className="mb-1 flex items-baseline gap-2">
        <span className="text-[12px] font-semibold text-foreground">Acceptance by draft position</span>
        <span className="text-[11px] text-ink-3">share of drafts accepted at each position, since llama started</span>
      </figcaption>
      <div className="relative" style={{ height: HEIGHT }}>
        <svg width="100%" height={HEIGHT} role="img" aria-label="Acceptance by draft position">
          <line x1="0" x2="100%" y1={HEIGHT - PAD_BOTTOM} y2={HEIGHT - PAD_BOTTOM} stroke="var(--rule)" strokeWidth={1} />
          {rates.map((r, i) => {
              ...
              >
                ...
                <svg x={`${slot * i}%`} width={`${slot}%`} height={HEIGHT} overflow="visible">
                  ...
                  <rect
                    ...
                    opacity={hover === null || hover === i ? 1 : 0.55}
                  />
                  <text x="50%" y={HEIGHT - 4} textAnchor="middle" className="fill-ink-3 text-[10px] tabular-nums">
                    {i + 1}
                  </text>
                  {(i === 0 || hover === i) && (
                    <text
      ...
      {hover !== null && (
        <p className="mt-1 text-[11px] text-ink-2" role="status">
          Position {hover + 1}: {shown[hover].toLocaleString()} accepted ({Math.round(rates[hover] * 100)}% of{' '}
          {Math.round(drafts).toLocaleString()} drafts)
        </p>
      )}
    </figure>
  )
