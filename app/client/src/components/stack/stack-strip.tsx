...
import { contextLevel, formatRatio, latestContext, weightedAcceptance, weightedDecodeTps } from '@/lib/stack-format'
...
export function StackStrip({ onOpen }: { onOpen: () => void }) {
  ...
  return (
    <button
      type="button"
      onClick={onOpen}
      title={unreachable || stalled ? `llama ${status.state}: ${status.lastError ?? ''}` : 'Inference stack'}
    ...
    >
      ...
      />
      {unreachable || stalled ? (
        <span className="truncate">{unreachable ? 'Stack unreachable' : 'llama queue stalled'}</span>
      ) : (
        <span className="flex min-w-0 flex-1 items-center gap-2 tabular-nums">
          ...
          {acceptance !== null && <span className="text-ink-3">draft {formatRatio(acceptance)}</span>}
          {context && (
            <span className="ml-auto inline-flex items-center gap-1 text-ink-3">
              <span className={`h-1.5 w-1.5 rounded-full ${DOT[contextLevel(context.percent)]}`} aria-hidden />
