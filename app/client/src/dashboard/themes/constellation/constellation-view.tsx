...
const PALETTE_STORAGE_KEY = 'instantcoffee-observe-constellation-palette'
const WINDOW_STORAGE_KEY = 'instantcoffee-observe-constellation-window'
const ZOOM_STORAGE_KEY = 'instantcoffee-observe-constellation-zoom'
const COLLAPSED_STORAGE_KEY = 'instantcoffee-observe-constellation-collapsed'
...
export function ConstellationView({ onOpenSession }: DashboardThemeProps) {
  ...
  useEffect(() => {
    ...
    const frame = () => {
      ...
      for (const m of metas) {
        ...
        if (els.pulseWrap) {
          els.pulseWrap.style.opacity = Math.max(0, h - 0.06).toFixed(3)
        }
        if (els.label) {
          els.label.style.opacity = (0.28 + 0.55 * h).toFixed(3)
        }
      }
  ...
  const showTooltip = (e: React.MouseEvent, m: NodeMeta) => {
    ...
    // Built with textContent, never innerHTML: slugs are pi session names and
    // project names come from directories — both reach the server through
    // unauthenticated POST /api/events.
    ...
    t.replaceChildren(
      ...
      row('subagents', String(m.orbitDots)),
  ...
  const onPointerMove = (e: React.PointerEvent) => {
    ...
    if (!movedRef.current) {
      if (Math.abs(dx) + Math.abs(dy) <= DRAG_THRESH) {
        return // still a click, not a pan
      }
