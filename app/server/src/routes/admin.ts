...
// Controlled by INSTANTCOFFEE_OBSERVE_ALLOW_DB_RESET: allow | deny | backup (default)
router.delete('/data', async (c) => {
  ...
  if (policy !== 'allow' && policy !== 'backup') {
    return apiError(c, 403, 'Database reset is disabled', {
      ...
      details: 'Set INSTANTCOFFEE_OBSERVE_ALLOW_DB_RESET=allow or backup to enable',
    })
  }
