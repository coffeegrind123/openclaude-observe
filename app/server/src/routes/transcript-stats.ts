...
router.get('/sessions/:sessionId/transcript-stats', async (c) => {
  ...
  try {
    // Lazy import: the transcript-parser pulls in the models.dev pricing
    // module graph. Keeping it out of the static import tree means the
    // server (and the test harness) doesn't pay that cost until the stats
    // endpoint is actually hit.
    const { parseSessionTranscripts } = await import('../transcript-parser')
