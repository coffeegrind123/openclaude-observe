...
export interface TranscriptCall {
  /** Cost the agent itself recorded for this request, in USD. When present it
   *  wins over models.dev pricing: pi prices from the operator's own model
   *  config, which is the only source that knows a local model costs 0. */
  costUsd?: number | null
  messageId: string
